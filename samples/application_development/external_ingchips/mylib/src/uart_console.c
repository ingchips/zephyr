...
void cmd_start(const char *param)
{
    // start_adv();
    // extern void setup_scan(void);
    // extern void setup_adv(int connectable);
    // setup_adv(0);
    // setup_scan();
}
...
void cmd_oob(const char *param) {
    //todo
    // uint8_t data[32];
    // uint8_t i = 0;
    // platform_printf("received uart_oob\r\n");
    // for (i = 0; i <32; i++) {
    //      sscanf(param + 2, "%2x", &data[i]);
    // }
    // set_peer_oob_data(data, 32);
