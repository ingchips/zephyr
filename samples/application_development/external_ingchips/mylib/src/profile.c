...
static void dump_32(const void *pp, int len)
{
    const uint32_t *p = (const uint32_t *)pp;
    int i;
    for (i = 0; i < len; i++)
        platform_printf("[%02d] = %08X\n", i, p[i]);
    platform_printf("\n");
}
...
#define DEF_NUM_OF_HCI              0x0b
    ...
    #include "hci_uart.c"
#elif (HCI_TRANSPORT == HCI_TRANSPORT_USB)
    #include "hci_usb.c"
#else
    #error Unsupported HCI transport
#endif
